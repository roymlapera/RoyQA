...
def Interpola(Curva, step):
    ...
    f = scipy.interpolate.interp1d(Curva.eje, Curva.valor)
    ...
    if(Curva.coordenada=='X' or Curva.coordenada=='Y'):
        ...
        eje = np.arange(2,Curva.eje.max(),step)
...
def ProcesaCurva(Curvas_med,Curvas_cal,espaciamiento,sigma_suavizado):
    ...
    for i in range(len(Curvas_cal)):
        Curvas_cal[i].eje,Curvas_cal[i].valor = Interpola(Curvas_cal[i],espaciamiento)    #datos cada 0.5 mm
        Curvas_cal[i].valor = SuavizaYNormaliza(Curvas_cal[i],sigma_suavizado)

    return Curvas_med, Curvas_cal

def Analiza_Y_Grafica_Perfiles(Perfiles_med,Perfiles_cal,dose_threshold,dta,cutoff):
    idx_med = np.arange( len(Perfiles_med) )
    idx_cal = np.arange( len(Perfiles_cal) )

    for i,j in itertools.product( idx_med , idx_cal ):    
        if(Perfiles_med[i].tc == Perfiles_cal[j].tc and 
        Perfiles_med[i].profundidad == Perfiles_cal[j].profundidad and
        Perfiles_med[i].dfp == Perfiles_cal[j].dfp and
        Perfiles_med[i].equipo == Perfiles_cal[j].equipo):
...
def Analiza_Y_Grafica_PDDs(PDDs_med,PDDs_cal,dose_threshold,dta,cutoff):
    idx_med = np.arange( len(PDDs_med) )
    idx_cal = np.arange( len(PDDs_cal) )

    for i,j in itertools.product( idx_med , idx_cal ):    
        if(PDDs_med[i].tc == PDDs_cal[j].tc and 
        PDDs_med[i].dfp == PDDs_cal[j].dfp and 
        PDDs_med[i].equipo == PDDs_cal[j].equipo):
...
def Grafica_Perfiles(Perfiles_med,Perfiles_cal,datos,i,dose_threshold,dta,cutoff):
    ...
    gamma = pymedphys.gamma(Perfiles_cal.eje, Perfiles_cal.valor, Perfiles_med.eje, Perfiles_med.valor,
                            dose_percent_threshold=dose_threshold, distance_mm_threshold=dta, lower_percent_dose_cutoff=cutoff)

    ax1.plot(Perfiles_cal.eje, Perfiles_cal.valor, marker='.', markersize=0, linewidth=1, label="Eclipse v13.7", color='b')
    ax1.plot(Perfiles_med.eje, Perfiles_med.valor, marker='.', markersize=2, linewidth=0, label=str(Perfiles_med.fecha), color='g')
    ax2.plot(Perfiles_cal.eje, gamma, marker='+', markersize=0, linewidth=0.5, label='Gamma '+str(dose_threshold)+'% '+str(dta)+'mm', color='r')
    ax2.axhline(y = 1 , color='r', linewidth=0.5)

    ax1.set_title(str(Perfiles_cal.equipo)+' - QA ANUAL '+str(Perfiles_med.fecha)[6:]+' - Perfil en '+str(Perfiles_med.coordenada)+
    ...
    ax1.text(min(Perfiles_med.eje)/3.5, 30, 
                    ...
                    'Penumbra izq: '+'('+str(datos[2])+' mm, '+str(datos[3])+' mm)'+'\n'
                    'Plateau:      '+'('+str(datos[4])+' %, '+str(datos[5])+' %)'+'\n'
                    'Penumbra der: '+'('+str(datos[6])+' mm, '+str(datos[7])+' mm)'+'\n'
                    'Umbra der:    '+'('+str(datos[8])+' %, '+str(datos[9])+' %)'+'\n'
                    '\n'
                    'Umbra<10%'+'\n'
                    '10%<Penumbra<90%'+'\n'
                    'Plateau<90%'+'\n', 
    ...
    ax1.set_xlim([max(Perfiles_med.eje.min(),Perfiles_cal.eje.min()), min(Perfiles_med.eje.max(),Perfiles_cal.eje.max())])
    ...
    fig.savefig('Perfil'+str(i)+'.pdf', orientation='landscape', papertype='a4', format='pdf', bbox_inches='tight')
...
def Grafica_PDDs(PDDs_med,PDDs_cal,i,dose_threshold,dta,cutoff):
    ...
    ax1.plot(PDDs_cal.eje, PDDs_cal.valor, marker='.', markersize=0, linewidth=1, label="Eclipse v13.7", color='b')
    ...
    fig.savefig('Pdd'+str(i)+'.pdf', orientation='landscape', papertype='a4', format='pdf', bbox_inches='tight')
