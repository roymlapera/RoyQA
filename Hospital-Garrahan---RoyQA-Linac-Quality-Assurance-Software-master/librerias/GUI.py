...
import sys
sys.path.append('C:/Users/howar/Desktop/Python/Tkinter/RoyQA/librerias')
import Perfiles as prof
...
class VentanaPrincipal:
    ...
    def __init__(self):     # cuando inicializo esta clase se crea una ventana con prop predeterminadas   
        ...
        logo = ImageTk.PhotoImage(Image.open("logo.jpg").resize((274, 117), Image.ANTIALIAS))    #importo imagen 
        ...
        filemenu.add_command(label='Análisis Starshots')
        filemenu.add_command(label='Análisis Picket Fence')             #.#
...
def AnalisisPerfiles():
    ...
    Perfiles_cal = prof.IngresaPerfilCalculado(filename_cal)
    Perfiles_med = prof.IngresaPerfilMedido(filename_med)

    Perfiles_med, Perfiles_cal = prof.ProcesaCurva(Perfiles_med,Perfiles_cal,espaciamiento=0.5,sigma_suavizado=0.00005)

    prof.Analiza_Y_Grafica_Perfiles(Perfiles_med,Perfiles_cal,dose_threshold,dta,cutoff)
...
def AnalisisPDDs():
    ...
    PDDs_cal = prof.IngresaPDDCalculado(filename_cal)
    PDDs_med = prof.IngresaPDDMedido(filename_med)


    PDDs_med, PDDs_cal = prof.ProcesaCurva(PDDs_med,PDDs_cal,espaciamiento=0.5,sigma_suavizado=0.0005)

    prof.Analiza_Y_Grafica_PDDs(PDDs_med,PDDs_cal,dose_threshold,dta,cutoff)
